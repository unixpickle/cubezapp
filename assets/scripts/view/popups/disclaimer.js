(function() {
  ...
  function DisclaimerPopup() {
    var $content = $('<div class="disclaimer-popup-content"></div>').text(
      'This website is still under development. We do not recommend using this website as your ' +
      'main timer until we release a more stable version.'
    );
    
    this._checkbox = window.app.flavors.makeCheckbox();
    
    var $checkboxContainer = $('<div class="disclaimer-popup-checkbox-container"></div>');
    $checkboxContainer.append(this._checkbox.element());
    $checkboxContainer.append('<label class="disclaimer-popup-checkbox-label">Never show this ' +
      'again</label>');
    $content.append($checkboxContainer);
    
    $checkboxContainer.find('.disclaimer-popup-checkbox-label').click(function() {
